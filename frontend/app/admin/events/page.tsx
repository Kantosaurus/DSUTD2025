...
function PendingEventsSection() {
  const [pendingEvents, setPendingEvents] = useState<any[]>([])
  const [loadingPending, setLoadingPending] = useState(false)
  const [showRejectModal, setShowRejectModal] = useState(false)
  const [selectedEventForReject, setSelectedEventForReject] = useState<any>(null)
  ...
  const approveEvent = async (eventId: number) => {
    try {
      ...
      if (response.ok) {
        ...
        alert('Error approving event: ' + error.error)
      }
    } catch (error) {
  ...
  const rejectEvent = async () => {
    if (!selectedEventForReject || !rejectionReason.trim()) return

    try {
      const token = localStorage.getItem('token')
      const response = await fetch(`${API_URL}/api/admin/events/${selectedEventForReject.id}/reject`, {
        method: 'POST',
        headers: { 
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (response.ok) {
        ...
        const error = await response.json()
        alert('Error rejecting event: ' + error.error)
      }
    } catch (error) {
  ...
  return (
    <div className="mb-12">
            ...
            <div key={event.id} className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-3 text-sm text-gray-500 mb-3">
                    <div>
                      <span className="font-medium">Creator:</span> {event.creator_student_id} ({event.creator_role})
                    </div>
                    <div>
                    ...
                    <div>
                      <span className="font-medium">Max:</span> {event.max_participants || '∞'}
                    </div>
                  </div>
                ...
                <div className="flex flex-col gap-2 ml-4">
                  ...
                  <button
                    onClick={() => {
                      setSelectedEventForReject(event)
      ...
      {/* Reject Event Modal */}
      {showRejectModal && selectedEventForReject && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-md w-full">
            <div className="p-6">
              <h3 className="text-lg font-semibold mb-4">Reject Event: {selectedEventForReject.title}</h3>
              <div className="mb-4">
